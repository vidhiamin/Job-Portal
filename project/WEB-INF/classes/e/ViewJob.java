				...
				for(Map.Entry<String,Job> entry : hm3.entrySet())
				{
					...
					if(job.getId().equals(id) && job.getCategory().equals(category) )
					{
						...
						out.println("  <div class=\"col-sm-12 col-md-6\">" +
									...
									"        <button><p><a href=\"AddApply?id="+job.getId()+"&&companyname="+job.getCompanyName()+"&&salary=" +job.getSalary()+"&&category=" +job.getCategory()+ "&&description="+job.getJobDescription()+"&&jobtype="+job.getJobType()+"&&jobposition="+job.getJobPosition()+"&&state="+job.getState()+"\" class=\"btnreview\">Apply</a></p></button>" +												
									"      </div>" +
									"    </div>" +
									"	</div>");
					}			   
